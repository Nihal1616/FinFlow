...
export default function SendMoney() {
  ...
  const selectRecipient = (u) => { setRecipient(u); setResults([]); setQuery(''); };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setAmountStr(''); setNote(''); clearRecipient();
    } catch (err) {
  ...
  return (
    <div className="animate-fade-in">
      ...
      <p style={{ color: 'var(--text2)', marginBottom: '1.5rem', fontSize: '0.9rem' }}>Transfer funds instantly to any FinFlow user</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', maxWidth: 900 }}>
        ...
        <div>
          ...
          <Card style={{ marginBottom: '1rem' }}>
            ...
            {results.length > 0 && (
              <div style={{ border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden', marginTop: 4 }}>
                {results.map(u => (
                  <div key={u._id} onClick={() => selectRecipient(u)} style={{
                  ...
                  >
                    <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'var(--accent3)',
            ...
            {recipient && (
              <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 10,
                padding: '1rem', display: 'flex', alignItems: 'center', gap: 12, marginTop: 8 }}>
                <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'linear-gradient(135deg,var(--accent),var(--green))',
                ...
                <button onClick={clearRecipient} style={{ background: 'none', border: '1px solid var(--border2)', borderRadius: 6,
                  color: 'var(--text2)', cursor: 'pointer', padding: '4px 10px' }}>×</button>
              </div>
            )}
          </Card>
          ...
          <Card>
            ...
            {isFraud && (
              <div style={{ background: 'rgba(255,179,71,0.1)', border: '1px solid rgba(255,179,71,0.4)',
                borderRadius: 10, padding: '0.8rem 1rem', marginBottom: '1rem', fontSize: '0.85rem', color: 'var(--amber)' }}>
                ⚠ Large transaction detected. This may trigger a security review.
              </div>
            )}
            <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '2.5rem', fontWeight: 800, textAlign: 'center', padding: '0.75rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, maxWidth: 280, margin: '0 auto 1.25rem' }}>
              {[1,2,3,4,5,6,7,8,9,'.','0','⌫'].map((k) => (
                <button key={k} onClick={() => k === '⌫' ? delKey() : addKey(k.toString())} style={{
                  background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 12,
                  padding: '0.9rem', fontSize: '1.1rem', fontFamily: 'Syne, sans-serif', fontWeight: 600,
        ...
        <div>
            ...
            {balance !== null && (
              <div style={{ marginTop: '1rem' }}>
                ...
                <div style={{ height: 6, background: 'var(--bg4)', borderRadius: 3, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: balance > 0 ? `${Math.max(0, Math.min(100, ((balance - amount) / balance) * 100))}%` : '0%',
                    background: 'linear-gradient(90deg,var(--accent),var(--accent2))', borderRadius: 3, transition: 'width 0.4s ease' }} />
                </div>
              </div>
            )}
          </Card>
          ...
          <Card style={{ marginBottom: '1rem' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {QUICK_AMOUNTS.map(a => (
                <button key={a} onClick={() => setQuick(a)} style={{
                  background: 'transparent', border: '1px solid var(--border2)', borderRadius: 8,
                  padding: '6px 14px', color: 'var(--text2)', cursor: 'pointer',
                  fontSize: '0.82rem', fontFamily: 'DM Sans, sans-serif', transition: 'all 0.2s',
                }}>₹{a.toLocaleString('en-IN')}</button>
              ))}
            </div>
          </Card>

          <Button variant="primary" full loading={loading} onClick={handleSend}
